...
shinyServer(function(input, output, session) {
  ...
  #Countries
    ...
    filteredcountry <- reactive({
      statadata  %>% dplyr::filter(CountryofLiving == input$country)  %>% summarize(n = n())
    ...
    filtereddatefirst <- reactive({
      statadata %>% dplyr::filter(CountryofLiving == input$country)  %>% dplyr::summarise(StartDate=min(StartDate)) %>% mutate(StartDate = format(as.Date(StartDate), "%d.%m.%Y"))
    ...
    filtereddatelast <- reactive({
      statadata %>% dplyr::filter(CountryofLiving == input$country)  %>% dplyr::summarise(StartDate = max(StartDate)) %>% mutate(StartDate = format(as.Date(StartDate), "%d.%m.%Y"))
    ...
    output$covidcasescum <- renderPlot({
      ...
      surveyfirstdate <- statadata %>% dplyr::filter(CountryofLiving == input$country)  %>% dplyr::summarise(StartDate= min(StartDate))
      surveylastdate <- statadata %>% dplyr::filter(CountryofLiving == input$country)   %>% dplyr::summarise(StartDate = max(StartDate))
    ...
    output$nocases <- renderValueBox({
      totalNoCases<- data %>% dplyr::filter(countriesAndTerritories==input$country) %>% dplyr::summarise_at("cases",sum) %>% prettyNum(big.mark = " ")
      valueBox(totalNoCases, "Number of confirmed cases", icon = icon("list"), color = "blue")
    ...
    output$nodeathsPercases <- renderValueBox({
      totalNoDeaths <- data  %>% dplyr::filter(countriesAndTerritories==input$country) %>% dplyr::summarise_at("deaths",sum)
      totalNoCases <- data  %>% dplyr::filter(countriesAndTerritories==input$country) %>% dplyr::summarise_at("cases",sum)
  ...
  #Continents  
    ...
    filteredcontinent <- reactive({
      ...
      statadata  %>% dplyr::filter(continent == input$continent)  %>% summarize(n = n())
    })
    ...
    filtereddatefirstcon <- reactive({
      statadata %>% dplyr::filter(continent == input$continent)  %>% dplyr::summarise(StartDate = min(StartDate)) %>% mutate(StartDate = format(as.Date(StartDate), "%d.%m.%Y"))
    ...
    filtereddatelastcon <- reactive({
      statadata %>% dplyr::filter(continent == input$continent)  %>% dplyr::summarise(StartDate = max(StartDate)) %>% mutate(StartDate = format(as.Date(StartDate), "%d.%m.%Y"))
    })
    ...
    populationofcountry2018 <- reactive({
      data %>% dplyr::filter(countriesAndTerritories == input$country) %>% distinct(popData2018) %>% prettyNum(big.mark = " ")
...
#    })
    ...
    output$nocasescon <- renderValueBox({
      ...
      valueBox(totalNoCases, "Number of confirmed cases", icon = icon("list"),
               color = "navy")
    ...
    data %>% dplyr::filter(continentExp == input$continent) %>% dplyr::summarise_at("popData2018",sum, na.rm = TRUE) %>% prettyNum(big.mark = " ")
    })
    ...
    output$nocasesPerPopcon <- renderValueBox({
      ...
      filteredCasesPop <- round((totalNoCases/ContinentPop$popData2018)*100000,1) 
      
      valueBox(if(filteredCasesPop < 1){"< 1"} else {filteredCasesPop}, "Number of confirmed cases per 100.000 inhabitants (2018)", icon = icon("list"),
               color = "navy")
    ...
    output$nodeathsPercasescon <- renderValueBox({
      totalNoDeaths <- data  %>% dplyr::filter(continentExp==input$continent) %>% dplyr::summarise_at("deaths",sum)
      totalNoCases <- data  %>% dplyr::filter(continentExp==input$continent) %>% dplyr::summarise_at("cases", sum)
