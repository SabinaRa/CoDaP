...
data <- read_excel("./data/COVID-19-geographic-disbtribution-worldwide-2020-04-29.xlsx")
...
statadata <- statadata %>% mutate(CountryofLiving =replace(CountryofLiving, CountryofLiving=="United States", "United States of America")) %>% mutate(CountryofLiving =replace(CountryofLiving, CountryofLiving=="Czech Republic", "Czechia")) %>% mutate(CountryofLiving =replace(CountryofLiving, CountryofLiving=="Northern Macedonia", "North Macedonia")) %>% mutate(CountryofLiving =replace(CountryofLiving, CountryofLiving=="Republic of Moldova", "Moldova")) %>% mutate(continent =replace(continent, continent=="Americas", "America")) 

#preparation to implement join
bycountry_data <- data %>% distinct(countriesAndTerritories,geoId)
bycountry_statadata <- statadata %>% group_by(CountryofLiving,iso2c) %>%  tally()  %>% dplyr::filter(n >=10)
...
bycontinent_statadata <- statadata %>% group_by(continent) %>%  tally()  %>% dplyr::filter(n >=10)
